...
# keep only rows where both_best_score is True
df = df[df["both_best_score"]]
...
for y in y_cols:
    ...
    # -------- BOTTOM STRIPES: disease × gene membership (uses filtered df) --------
    ...
    # --- TOP: barplot (single color; all rows are True) ---
    ax_top = fig.add_subplot(gs[0, 0])
    sns.barplot(
        data=top, x="Gene", y=y,
        color="crimson",               # single color since all True
    ...
    # save
    safe_y = y.replace(" ", "_")
    outfile = os.path.join(outdir, f'FILTERED_genes__{safe_y}__with_disease_stripes.png')
