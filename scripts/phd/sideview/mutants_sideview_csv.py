...
def cluster_percentage(parent_folder):
    ...
    for condition in os.listdir(parent_folder):
        ...
        # normal case:
        # parent_folder/CONDITION/clustering
        cluster_folder = os.path.join(condition_path, "clustering")

        # non-tb case:
        # parent_folder/CONDITION/non-tb/clustering
        if not os.path.isdir(cluster_folder):
            cluster_folder = os.path.join(condition_path, "non-tb", "clustering")

        if not os.path.isdir(cluster_folder):
            print(f"Skipping {condition} as it does not contain a 'clustering' or 'non-tb/clustering' folder.")
            continue
...
def nearest_neighbour(parent_folder):
    ...
    for condition in os.listdir(parent_folder):
        ...
        for cluster_file in cluster_files:
            ...
            df = df[df["frame"] % 50 == 0].copy() ## sampling every 50 frames, 10 second
...
def cluster_size_vs_nearest_neighbour(parent_folder):
    ...
    for condition in os.listdir(parent_folder):
        ...
        for cluster_file in cluster_files:
            ...
            df = pd.read_feather(os.path.join(cluster_folder, cluster_file))
            df["frame"] = df["frame"].astype(int)
            df = df[df["frame"] % 50 == 0].copy() ## sampling every 50 frames, 10 seconds
...
parent_folder = '/Volumes/lab-windingm/home/users/cochral/PhD/NDD/EXPERIMENTS/NXR-MUTANT'
