...
output = '/Users/cochral/repos/behavioural-analysis/plots/phd/mutants'

################################################################
######## --------- PLOT PERCENTAGE IN CLUSTERS -------- ########
################################################################

df = pd.read_csv('/Volumes/lab-windingm/home/users/cochral/PhD/NDD/EXPERIMENTS/NXR-MUTANT/ANALYSIS/percent_in_clusters.csv')
...
df = pd.read_csv('/Volumes/lab-windingm/home/users/cochral/PhD/NDD/EXPERIMENTS/NXR-MUTANT/ANALYSIS/n_clusters_over_time.csv')
...
df = pd.read_csv('/Volumes/lab-windingm/home/users/cochral/PhD/NDD/EXPERIMENTS/NXR-MUTANT/ANALYSIS/avg_cluster_size_over_time.csv')
...
################################################################
df = pd.read_csv('/Volumes/lab-windingm/home/users/cochral/PhD/NDD/EXPERIMENTS/NXR-MUTANT/ANALYSIS/max_cluster_size_over_time.csv')
...
################################################################
df = pd.read_csv('/Volumes/lab-windingm/home/users/cochral/PhD/NDD/EXPERIMENTS/NXR-MUTANT/ANALYSIS/depth_difference_food.csv')
...
################################################################
# df = pd.read_csv('/Volumes/lab-windingm/home/users/cochral/PhD/NDD/EXPERIMENTS/ELAV_T21-D02/SIDEVIEW/ANALYSIS/nearest_neighbour.csv')
...
df = pd.read_csv('/Volumes/lab-windingm/home/users/cochral/PhD/NDD/EXPERIMENTS/NXR-MUTANT/ANALYSIS/nearest_neighbour.csv')
print('Nearest neighbour data loaded:')

bin_size = 300  # 5 fps * 60 s
df["frame_bin"] = (df["frame"] // bin_size) * bin_size
...
control = "T23-A08"
conditions = sorted([c for c in df["condition"].unique() if c != control])

for nn_col in nn_columns:

    nearest_neighbour = (
        df.groupby(["condition", "file_id", "frame_bin"], as_index=False)[nn_col]
          .mean()
          .rename(columns={"frame_bin": "frame"})
    )

    for cond in conditions:
        print(f"Plotting {cond} - {nn_col}...")

        plot_df = nearest_neighbour[
            nearest_neighbour["condition"].isin([control, cond])
        ].copy()

        plot_df["condition"] = pd.Categorical(
            plot_df["condition"],
            categories=[control, cond],
            ordered=True
        )
        ...
        plt.title(nn_col.replace("_", " ").title())
        plt.legend(title="Condition")
        sns.despine()

        outpath = os.path.join(output, f"{cond}__{nn_col}.pdf")
        plt.savefig(outpath, format="pdf", bbox_inches="tight")
        plt.close()
...
################################################################
df = pd.read_csv('/Volumes/lab-windingm/home/users/cochral/PhD/NDD/EXPERIMENTS/NXR-MUTANT/ANALYSIS/cluster_size_vs_nearest_neighbour.csv')
...
for cond in conditions:
    print(f"Plotting {cond}...")
    plot_df = df[df["condition"].isin([control, cond])].copy()
    plot_df["condition"] = pd.Categorical(plot_df["condition"], categories=[control, cond], ordered=True)
    ...
    plt.title("Cluster Size vs Within-Cluster Spacing")

    sns.despine()

    outpath = os.path.join(output, f"{cond}__cluster_size-vs-nearest_neighbour.pdf")
    plt.savefig(outpath, format="pdf", bbox_inches="tight")
    plt.close()
