...
#             #array = [j, i, getattr(instance, "score", np.nan)] + list(instance.points_and_scores_array.flatten())
#             if hasattr(instance, "points_and_scores_array"):
#                 array = [j, i, getattr(instance, "score", np.nan)] + list(instance.points_and_scores_array.flatten())
#                 array = [np.round(x, 2).astype('float32') for x in array]
#                 data.append(array)
#             else:
#                 print(f"⚠️ Skipping instance in frame {i} (track {j}) — no points found.")
...
#     data = []
#     for frame in label_obj.labeled_frames:
#         frame_idx = frame.frame_idx  # Use true frame number, not index `i`

#         for instance in frame.instances:
#             if not hasattr(instance, "points_and_scores_array"):
#                 print(f"⚠️ Skipping frame {frame_idx} — no points for instance.")
#                 continue

#             if track_ids:
#                 try:
#                     track_id = int(instance.track.name.replace("track_", ""))
#                 except Exception as e:
#                     print(f"⚠️ Could not parse track ID at frame {frame_idx}: {e}")
#                     track_id = -1
#             else:
#                 track_id = 0

#             array = [track_id, frame_idx, getattr(instance, "score", np.nan)] + list(instance.points_and_scores_array.flatten())
#             # array = [np.round(x, 2).astype('float32') for x in array]
...
### THIRD
def slp_to_feather(file_path, skel_parts, track_ids=True):
    feather_file = file_path.replace(".slp", ".feather")
    labels = read(file_path, for_object="labels")
    ...
    for frame in labels.labeled_frames:
        ...
        for inst in frame.instances:
            # Determine track_id
            if track_ids and inst.track is not None:
                try:
                    track_id = int(inst.track.name.replace("track_", ""))
                except:
                    track_id = -1
            else:
                track_id = 0

            is_numpy = hasattr(inst.points[0], "__getitem__") if inst.points else False

            # Check if it's an edited instance
            if is_numpy:
                has_score_field = "score" in inst.points[0].dtype.names
                score_value = inst.points[0]["score"] if has_score_field else None
                is_edit = not has_score_field or score_value is None
            else:
                is_edit = getattr(inst, "score", None) is None

            # Add first instance or prefer edited version
            if track_id not in track_dict:
                track_dict[track_id] = inst
            elif is_edit:
                track_dict[track_id] = inst  # Overwrite prediction with edit

        # Export one instance per track per frame
        for track_id, inst in track_dict.items():
            is_numpy = hasattr(inst.points[0], "__getitem__") if inst.points else False

            coords = []
            for pt in inst.points:
                if is_numpy:
                    x = float(pt["x"]) if pt["x"] is not None else np.nan
                    y = float(pt["y"]) if pt["y"] is not None else np.nan
                    s = float(pt["score"]) if "score" in pt.dtype.names and pt["score"] is not None else np.nan
                else:
                    x = float(pt.x) if pt.x is not None else np.nan
                    y = float(pt.y) if pt.y is not None else np.nan
                    s = float(pt.score) if pt.score is not None else np.nan
                coords.extend([x, y, s])

            row = [track_id, frame_idx, getattr(inst, "score", np.nan)] + coords
            data.append(row)
...
# Special case: convert tracks.slp to feather without affecting rest of the logic
if 'x' in job:  # x = "just convert tracks"
    video_file_paths = [x.replace('.mp4', '.tracks.slp') for x in video_file_paths]
    job += 'c'  # make sure conversion runs
