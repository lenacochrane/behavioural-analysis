...
class HoleAnalysis:
   ...
   # METHOD SHORTEN: OPTIONAL METHOD TO SHORTEN THE TRACK FILES TO INCLUDE UP TO A CERTAIN FRAME  
    ...
    #             for interaction_type, (positions1, positions2) in { ##wrong there shd be 9 here
    #                         {
    ...
    #                         'tail_head': (tail_positions, head_positions),
    #                         'body_head': (body_positions, head_positions),
    #                         'head_body': (head_positions, body_positions),
    #                         'tail_body': (tail_positions, body_positions),
    ...
    #                     # Mask to exclude same-animal pairs
    #                     mask = ids1[:, None] != ids2[None, :]

    #                     if positions1 is positions2:
    #                         # For symmetric interactions (same array), avoid double counting
    #                         upper_triangle = np.triu((distances < threshold) & mask, k=1)
    #                         interaction_counts[interaction_type] += np.sum(upper_triangle)
    #                     else:
    #                         interaction_counts[interaction_type] += np.sum((distances < threshold) & mask)
