...
for n in neighbors_list:
    for d in min_dist_list:
        ...
        # Save UMAP plots by condition
        for cond in ["iso", "group"]:
            cond_df = umap_df[umap_df["condition"] == cond]
            plt.figure(figsize=(7, 5))
            sns.scatterplot(data=cond_df, x="UMAP1", y="UMAP2", alpha=0.8)
            plt.title(f"UMAP projection ({cond}) – n_neighbors={n}, min_dist={d}")
            plt.xlabel("UMAP Dimension 1")
            plt.ylabel("UMAP Dimension 2")
            plt.tight_layout()
            cond_fname = f"umap_n{n}_d{d}_{cond}.png"
            plt.savefig(os.path.join(output_dir, cond_fname))
            plt.close()
