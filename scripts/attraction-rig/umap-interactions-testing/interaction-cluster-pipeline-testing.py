...
#### here 
    ...
    palette = {
    "group": "C0",   # seaborn/matplotlib's first default blue
    "iso": "C1"      # seaborn/matplotlib's second default orange
}
    ...
    #### 5. BARPLOT OF PROPORTIONS PER CLUSTER AVERAGED (WITHIN CONDITION)

    # === Build per-video, per-cluster proportions (tidy long format) ===
    ...
    summary_csv_path = os.path.join(output_dir, 'per_video_cluster_proportions_long.csv')
    ...
    interaction_starts = df[df['Normalized Frame'] == 0].copy()
    ...
    # Save tidy table
    summary_csv_path = os.path.join(output_dir, 'per_video_cluster_proportions_by_timebin.csv')
    summary_df.to_csv(summary_csv_path, index=False)
    ...
    plt.tight_layout()
    timebin_prop_path = os.path.join(output_dir, 'cluster_proportions_by_timebin.png')
    ...
    for idx, cluster_id in enumerate(cluster_ids):
        ...
        ### SUMMARY ROW 6 ON AP
        ax8 = axes_ap[8, idx]
        ax8.plot(mean_dist.index, mean_dist.values, color='black')
        ax8.fill_between(mean_dist.index, mean_dist - std_dist, mean_dist + std_dist, color='gray', alpha=0.3)
    ...
    ##### SUMMARY ROW 12: NUMBER OF CONTACT <1MM FRAMES 
    ...
    for idx, cluster_id in enumerate(sorted(df[cluster_name].unique())):
        ...
        ax10 = axes_ap[12, idx]
        ax10.bar(0, mean_val, yerr=std_val, color='green', alpha=0.8, capsize=5)
        ax10.text(0, mean_val + 1, f"{mean_val:.1f}", ha='left', fontsize=16)

        ax10.set_ylim(0, 15)  # ✅ As requested
        ax10.set_xticks([])
        # ax_sum.set_yticks([])
    ...
    # === Initial & Predominant contact type per interaction (merged) ===
    contact_labels = []
    ...
    for cluster_id in sorted(df[cluster_name].unique()):
        ...
        for inter_id in cluster_df["interaction_id"].unique():
            sub = (cluster_df[cluster_df["interaction_id"] == inter_id]
                .sort_values("Frame").copy())

            # contact frames only
            contact = sub[sub["min_distance"] < 1].copy()
            if contact.empty:
                # no contact detected; skip (or label as None)
                continue

            # merge symmetrical labels
            contact["type_merged"] = contact["interaction_type"].map(interaction_merge_map)

            # INITIAL contact = first contact frame's merged type
            initial_type = contact["type_merged"].iloc[0]


            # PREDOMINANT contact = mode of merged type across contact frames
            # tie-break by first occurrence in value_counts (idxmax)
            predominant_type = contact["type_merged"].value_counts().idxmax()
    ...
    df_contact_labels = pd.DataFrame(contact_labels)
    ...
    for idx, cluster_id in enumerate(sorted(df_contacts["cluster"].unique())):
        ...
        ax9.set_ylim(0, stds.values.max() + means.values.max() * 0.5)  # or whatever consistent limit you like
        ax9.set_xticks([])   # if you really want no ticks
        ax9.set_visible(True)


        palette_list = [interaction_colors[t] for t in interaction_types]

        lab_sub = df_contact_labels[df_contact_labels["cluster"] == cluster_id].copy()
        if not lab_sub.empty:
            # Build 0/1 tables: one row per interaction per contact_type
            # Initial contact
            tmp_init = pd.DataFrame({
                "contact_type": np.repeat(interaction_types, len(lab_sub)),
                "val": np.concatenate([(lab_sub["initial_type"] == t).astype(int).values
            ...
            tmp_pred = pd.DataFrame({
                "contact_type": np.repeat(interaction_types, len(lab_sub)),
                "val": np.concatenate([(lab_sub["predominant_type"] == t).astype(int).values
                                    for t in interaction_types])
            })

            # --- Row 10: Initial Contact (% with SD across interactions)
            ax_init = axes_ap[10, idx]
            sns.barplot(
                data=tmp_init, x="contact_type", y="val",
                estimator=np.mean, errorbar="sd",
                order=interaction_types, palette=palette_list, ax=ax_init
            )
            ax_init.set_ylim(0, 1)
            ax_init.set_xticks([])
            ax_init.set_visible(True)

            # --- Row 11: Predominant Contact (% with SD across interactions)
            ax_pred = axes_ap[11, idx]
            sns.barplot(
                data=tmp_pred, x="contact_type", y="val",
                estimator=np.mean, errorbar="sd",
                order=interaction_types, palette=palette_list, ax=ax_pred
            )
...
output_dir = "/Volumes/lab-windingm/home/users/cochral/AttractionRig/analysis/social-isolation/n10/umap-pipeline/youngser/test3_F18/Yhat.idt.pca" 
