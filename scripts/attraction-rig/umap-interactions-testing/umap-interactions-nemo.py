...
output_dir = "/camp/lab/windingm/home/users/cochral/AttractionRig/analysis/social-isolation/n10/umap-pipeline"
...
ax = sns.scatterplot(
    ...
    palette="tab10",
    alpha=0.8,
    legend=False   # <- turn off the built-in legend for now
)

# now create a separate legend, placed outside the axes
# note: handles & labels from your scatter:
handles, labels = ax.get_legend_handles_labels()
leg = ax.figure.legend(
    handles, labels,
    title="Cluster",
    bbox_to_anchor=(1.02, 1),    # put it just to the right of the axes
    loc="upper left",
    borderaxespad=0,
    ncol=1,                      # you can tile it in multiple columns if you like
    frameon=False
)
