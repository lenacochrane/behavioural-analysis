...
labels = read("/Volumes/lab-windingm/home/users/cochral/AttractionRig/analysis/social-isolation/n2/group-housed/2025-02-24_17-42-09_td7.tracks.slp", for_object="labels")
  # << change this to your .slp file path
frame = labels.labeled_frames[0]

for i, inst in enumerate(frame.instances):
    tid = inst.track.name if inst.track else "None"
    print(f"Instance {i} — Track: {tid}, Score: {getattr(inst, 'score', None)}")
    print(f"  # Points: {len(inst.points)}")
    for pt in inst.points:
        print(f"    x: {pt.x}, y: {pt.y}, score: {pt.score}")
